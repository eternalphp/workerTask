...
class Worker{
		...
		Console::loop(function(){
			...
			if(Worker::$taskList){
				foreach(Worker::$taskList as $k=>$val){
					//监听启动状态
					if($val["status"] == Worker::STATUS_STARTING){
						Worker::createProcess($val["taskid"]); //创建服务进程
					}
				}
			}
	...
	public static function createProcess($taskid){
        ...
        $pipes       = array();
        $process     = proc_open("php \"$start_file\" -q", $descriptorspec, $pipes); //创建子进程
        $std_handler = fopen($std_file, 'a+');
        stream_set_blocking($std_handler, 0);
		$status = proc_get_status($process);
		
		fwrite($pipes[0],json_encode(self::$taskList[$taskid])); //给进程传递参数
		
		exec(sprintf("wmic process where ParentProcessId=%d get ProcessId",$status["pid"]),$result); //获取子进程的父进程pid

		Worker::$taskProcessList[$taskid] = array($process,$pipes);
		
		foreach(self::$taskList as $id=>$val){
			if($val["taskid"] == $taskid){
				$val["running"] = $status["running"];
				$val["ppid"] = $result[1];
				$val["pid"] = $status["pid"]; // 父进程id,子进程id
				...
				Worker::updateWorkerStatus(self::$taskList);
	...
	public static function stop(){
		...
		if($taskid > 0){
			...
			if($list){
				foreach($list as $taskid=>$val){
					if($val["running"] == 1){
						//@exec(sprintf("kill -9 %d -f",$val["pid"]));
						@exec(sprintf("kill -9 %d -f",$val["ppid"]));
					}
				}
			}
			sleep(1);
	...
	public static function status(){
		...
		if($list){
			...
			foreach($list as $k=>$val){
				...
				$line[] = implode(" | ",array(self::column($val["taskid"],10),self::column($val["ppid"].'/'.$val["pid"],10),self::column($val["title"],15),self::column($val["status"],10),self::column($val["running"],10),self::column($val["runtime"],20),self::column($val["stoptime"],20)));
			}
			$line[] = "-------------------------------------------------------------------------------------------------------------";
			if($line){
				echo "\n";
